...
tf.app.flags.DEFINE_integer('refresh_target_model_num', 10000,  # update the target Q model every refresh_target_model_num
                            """frames over which to anneal epsilon.""")
tf.app.flags.DEFINE_integer('replay_memory', 400,  # takes up to 20 GB to store this amount of history data
...
def test():
    ...
    # model = load_model(FLAGS.restore_file_path)
    model = load_model(FLAGS.restore_file_path,
                       custom_objects={'_huber_loss': huber_loss})  # load model with customized loss func
